...
export function ContactContent({ settings }: { settings: LiveSettings }) {
  const mapsUrl = `https://maps.google.com/?q=${encodeURIComponent(settings.address)}`;

  const contactDetails = [
    { label: "Phone", value: settings.phone, icon: "phone" as const },
    { label: "Email", value: settings.email, icon: "mail" as const },
    { label: "Studio", value: settings.address, icon: "pin" as const },
  ];

  const contactLinks = [
    {
      label: "WhatsApp",
      href: buildWhatsAppLink("Hi! I'd love to talk about my wedding date.", settings.whatsapp),
      icon: "whatsapp" as const,
    },
    { label: "Instagram", href: settings.instagram, icon: "instagram" as const },
    { label: "Google Maps", href: mapsUrl, icon: "pin" as const },
  ];

  return (
    <main>
      <section className="relative flex h-[55svh] min-h-[420px] w-full items-end overflow-hidden bg-black">
        <motion.div variants={expandReveal} initial="hidden" animate="visible" className="absolute inset-0">
          <Image
            src={brand.makeoverBrideCloseup}
            alt="Bridal jewelry and styling detail"
            fill
            priority
